...
export mkConfig : List (String, Parser a) -> SubparserConfig a
mkConfig cmds =
  MkSubparserConfig cmds Nothing

||| Set the program description for a subparser.
export progDesc : String -> SubparserConfig a -> SubparserConfig a
progDesc desc (MkSubparserConfig cmds _) =
  MkSubparserConfig cmds (Just desc)

||| Build a subparser from configuration.
export mkSubparser : SubparserConfig a -> Parser a
mkSubparser (MkSubparserConfig cmds _) =
  cmdList cmds
  where
